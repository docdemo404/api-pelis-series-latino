...
| **Un vídeo que reproduce perfectamente y NO es la obra** | `voe.sx` nos devolvió diez segundos de Big Buck Bunny —un mp4 real, sano, cinco megas— en **83 de 83** extracciones. Pasó la verificación ENTERA (resolver → manifiesto → segmento) porque todo eso comprueba que **lleguen bytes de vídeo**, no que sean **EL** vídeo. 94 fichas se anunciaron con un conejo dentro y lo detectó el usuario, no el código. Guarda genérica en `esVideoDeMuestra` (`directStream.ts`), que vale para todas las fuentes; `voe.sx` además entró en `DECOY_HOSTS`. Limpieza: `scripts/dev/limpiar_senuelos.ts`. |
